...
class SecureTextServer:
    ...
    def user_backward (self):
        all_users = list(self.users.keys())
        for item in all_users:
            print(item)
            try:
                salt=self.users[item]['saltpassword']
            except Exception as e:
                salt=bcrypt.gensalt()
                self.users[item]['saltpassword']=salt.decode()
                hashed = bcrypt.hashpw(self.users[item]['password'].encode('utf-8'),salt)
                self.users[item]['password']=hashed.decode()     
    ...
    def create_account(self, username, password):
        ...
        start = time.time()
        hash_object = hashlib.sha256(password.encode())  # convert string to bytes
        end = time.time()
        print("Time taken sha256: {:.4f} seconds".format(end - start))
        hex_dig = hash_object.hexdigest()
        
        salt = bcrypt.gensalt()
        print("salt size: ", len(salt))
        start = time.time()
        hashed = bcrypt.hashpw(password.encode('utf-8'), salt)
        end = time.time()
        print("Time taken bcrypt: {:.4f} seconds".format(end - start))
        self.users[username] = {
            'password': hashed.decode(),  # PLAINTEXT PASSWORD!
            'saltpassword': salt.decode(),
    ...
    def authenticate(self, username, password):
        ...
        salt = self.users[username]['saltpassword'].encode()
        hashed = bcrypt.hashpw(password.encode('utf-8'), salt)
        
        if self.users[username]['password'] == hashed.decode():
...
class SecureTextClient:
            ...
            try:
                ...
                if data:
                    ...
                    if message.get('type') == 'MESSAGE':
                        ...
                        content_mac=message['mac']
                        print(content_mac)
                        print(f"\n")
                        calculated_mac = self.attach_mac("default_key",content)
                        print(calculated_mac)
                        if(calculated_mac==content_mac):
                            try:
    ...
    def login(self):
        ...
        if response['status'] == 'success':
            ...
            listen_thread.start()
    def mac_md5(self,key: bytes, message: bytes) -> bytes:
        return hmac.new(key, message, hashlib.sha256).hexdigest()
    def attach_mac(self,key, message):
        mac = self.mac_md5(key.encode(), message.encode())
        return mac # Appending MAC to message
    def send_message(self):
        ...
        if not recipient or not content:
            ...
            return
        mac = self.attach_mac("default_key",content)
        print(mac)
        command = {
